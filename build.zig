...
pub fn build(b: *std.Build) !void {
    ...
    inline for ([_][]const u8{
        ...
        const run_cmd_name = try std.fmt.allocPrint(allocator, "run-{s}", .{demo_name});
        const run_cmd_description = try std.fmt.allocPrint(allocator, "Run demo {s}", .{demo_name});

        // This creates a build step. It will be visible in the `zig build --help` menu,
        // and can be selected like this: `zig build run`
        // This will evaluate the `run` step rather than the default, which is "install".
        const run_step = b.step(run_cmd_name, run_cmd_description);
